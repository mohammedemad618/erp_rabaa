...
type CrmDetailTab = "transactions" | "services" | "travel";
...
export function CrmConsole({ dataset }: CrmConsoleProps) {
  ...
  const [detailTab, setDetailTab] = useState<CrmDetailTab>("transactions");
  ...
  if (dataset.customers.length === 0) {
    return (
      <ErpPageLayout>
        <ErpPageHeader title={tCrm("title")} description={tCrm("subtitle")} actions={createCustomerAction} />
        <section className="surface-card col-span-12 p-6">
          <EmptyState
            variant="no-data"
            title={isAr ? "لا توجد بيانات عملاء بعد" : "No customer data yet"}
            description={
              isAr
                ? "ابدأ بإضافة معاملات أو حجوزات لعرض مؤشرات CRM."
                : "Add transactions or bookings to start populating CRM insights."
            }
            action={(
              <Link
                href="/operations"
              ...
              <section className="surface-card overflow-hidden">
                <header className="border-b border-border bg-slate-50 px-4 py-3">
                  <div className="flex flex-wrap items-center gap-2">
                    {([
                      { id: "transactions" as CrmDetailTab, label: locale === "ar" ? "المعاملات" : "Transactions", count: displayedTimeline.length },
                      { id: "services" as CrmDetailTab, label: locale === "ar" ? "الخدمات" : "Services", count: selectedCustomer.serviceBookings.length },
                      { id: "travel" as CrmDetailTab, label: locale === "ar" ? "طلبات السفر" : "Travel", count: selectedCustomer.travelRequests.length },
                    ]).map((tab) => (
                      <Button
                        key={tab.id}
                        size="sm"
                      ...
                      >
                        {tab.label}
                        {tab.count > 0 && (
                          <span className={`ms-1.5 rounded-full px-1.5 py-0.5 text-[9px] font-bold ${
                            detailTab === tab.id ? "bg-white/25 text-white" : "bg-slate-200 text-slate-600"
                          }`}>
                            {tab.count}
                          </span>
                        )}
                    ...
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-white">
                        <tr className="border-b border-border text-muted-foreground">
                          ...
                          <th className="px-2 py-2 text-start">{locale === "ar" ? "الخدمة" : "Service"}</th>
                          <th className="px-2 py-2 text-end">{locale === "ar" ? "المبلغ" : "Amount"}</th>
                          <th className="px-2 py-2 text-start">{locale === "ar" ? "الحالة" : "Status"}</th>
                          <th className="px-2 py-2 text-start">{locale === "ar" ? "التاريخ" : "Date"}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {selectedCustomer.serviceBookings.map((b) => {
                          const catInfo = SERVICE_CATEGORIES.find((c) => c.id === b.category);
                          return (
                            <tr key={b.id} className="border-b border-border/70">
                              <td className="px-2 py-2 font-medium text-finance">{b.id}</td>
                              <td className="px-2 py-2">
                              ...
                              <td className="px-2 py-2">{formatDate(b.createdAt, locale)}</td>
                            </tr>
                          );
                        })}
                        {!selectedCustomer.serviceBookings.length ? (
                          <tr>
                            <td colSpan={5} className="px-2 py-8 text-center text-muted-foreground">
                              {locale === "ar" ? "لا توجد حجوزات خدمات" : "No service bookings"}
                            </td>
                          </tr>
                        ) : null}
                      </tbody>
                  ...
                  <div className="max-h-[340px] overflow-auto">
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-white">
                        <tr className="border-b border-border text-muted-foreground">
                          <th className="px-2 py-2 text-start">ID</th>
                          <th className="px-2 py-2 text-start">{locale === "ar" ? "الوجهة" : "Destination"}</th>
                          <th className="px-2 py-2 text-end">{locale === "ar" ? "التكلفة" : "Cost"}</th>
                          <th className="px-2 py-2 text-start">{locale === "ar" ? "الحالة" : "Status"}</th>
                          <th className="px-2 py-2 text-start">{locale === "ar" ? "المغادرة" : "Departure"}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {selectedCustomer.travelRequests.map((tr) => (
                          <tr key={tr.id} className="border-b border-border/70">
                            <td className="px-2 py-2 font-medium text-finance">{tr.id}</td>
                            <td className="px-2 py-2">{tr.origin} → {tr.destination}</td>
                            <td className="px-2 py-2 text-end font-medium text-finance">
                              {formatCurrency(tr.estimatedCost, locale, tr.currency)}
                            </td>
                            <td className="px-2 py-2">
                              <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${
                                tr.status === "booked" ? "bg-emerald-100 text-emerald-700" :
                                tr.status === "closed" ? "bg-teal-100 text-teal-700" :
                            ...
                            <td className="px-2 py-2">{formatDate(tr.departureDate, locale)}</td>
                          </tr>
                        ))}
                        {!selectedCustomer.travelRequests.length ? (
                          <tr>
                            <td colSpan={5} className="px-2 py-8 text-center text-muted-foreground">
                              {locale === "ar" ? "لا توجد طلبات سفر" : "No travel requests"}
                            </td>
                          </tr>
                        ) : null}
                      </tbody>
